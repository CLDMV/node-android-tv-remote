 ...
 *
 * Usage: node tools/dual/device2.mjs
 */

import createRemote from "../../src/lib/android-tv-remote.mjs";

const DEVICE_IP = "10.6.0.18";
...
/**
 * Main test function for device 2
 */
const testDevice2 = async () => {
	const startTime = Date.now();
	let setup = null;

	try {
		log("INFO", "=== Device 2 Server Starting ===");
		...
		log("SUCCESS", `=== Device 2 Server Completed Successfully (${totalTime}ms) ===`);

		process.exit(0);
	} catch (error) {
		const totalTime = Date.now() - startTime;
		log("ERROR", `=== Device 2 Server Failed (${totalTime}ms) ===`, {
			error: error.message,
			stack: error.stack
		});
...
// Start the test
log("INFO", "Starting Device 2 test process...");
testDevice2();
